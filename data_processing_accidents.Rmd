...
##load files
```{r}
accidents <- read.csv("2022_accidents_gu_bcn.csv")

persones <- read.csv("2022_accidents_persones_gu_bcn.csv")

vehicles <- read.csv("2022_accidents_vehicles_gu_bcn.csv")
```
...
colnames_vehicles <- colnames(vehicles)

# Encontrar los nombres de columna comunes a todas las tablas
common_names <- Reduce(intersect, list(colnames_accidents, colnames_persones, colnames_vehicles))
...
unique_names_vehicles <- setdiff(colnames_vehicles, common_names)
```
...
```{r}
print("Nombres de columna únicos en vehicles:")
print(unique_names_vehicles)
```
...
merged_data <- merge(merged_data, vehicles, by = key_column)
